...
bool Manager::insertProcess(Process p, bool addToHistory)
{
	if (addToHistory)
		processHistory.push_back(p);
...
void Manager::refresh()
{
	...
	lastPosition = std::make_tuple(-1, 0);
	
	for (const auto& p : processHistory)
		insertProcess(p, false);
}
