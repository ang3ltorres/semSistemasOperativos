...
from final import Process, OrderBy, Manager, Algorithm
...
class MainWindow(QMainWindow):
	...
	def insert(self, name: str, priority: int, time: int, end: bool):
			...
			old_process = None
			if len(self.manager.process):
				old_process = self.manager.process[self.manager.index]
			...
			if (old_process):
				for i in range(len(self.manager.process)):
					if self.manager.process[i] is old_process:
						self.manager.index = i
